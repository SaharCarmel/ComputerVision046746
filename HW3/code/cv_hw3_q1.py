...
#                            functions                                        #
###############################################################################
def load_images(fp, named=False, method='PIL', rot=0):
    # desc
...
def wrapper_grabcut(named_img_list, colors_palette):
    ...
    for fn, img in named_img_list:
        ...
        cv2.grabCut(img, mask, rct, bgd_model, fgd_model,
...
def main():
    ...
    model_deeplab = torch.hub.load(
        'pytorch/vision:v0.5.0', 'deeplabv3_resnet101', pretrained=True)
    model_deeplab.eval()
